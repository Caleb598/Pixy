...
std::vector<Point> evaluateBSplineCurve(const std::vector<Point>& controlPoints, const std::vector<float>& knotVector,
                                        int degree, int parameterRange = 100) {
    ...
    for (int i = degree; i <= n; i++) {
        for (int j = 0; j <= parameterRange; j++) {
            float t = static_cast<float>(j) / parameterRange;
...
int main() {
    ...
    std::vector<float> knotVector = {0, 0, 0, 1, 2, 3, 4, 4, 4};
    int degree = 3;

    std::vector<Point> curvePoints = evaluateBSplineCurve(controlPoints, knotVector, degree);
