...
Point evaluateRationalNURBSSurface(const std::vector<std::vector<ControlPoint>>& controlPoints,
...
std::vector<std::vector<Point>> generateRationalNURBSSurface(const std::vector<std::vector<ControlPoint>>& controlPoints,
                                                             const std::vector<float>& knotVectorU, int degreeU,
                                                             const std::vector<float>& knotVectorV, int degreeV,
    ...
    for (int i = 0; i <= numPointsU; ++i) {
        ...
        for (int j = 0; j <= numPointsV; ++j) {
            ...
            Point point = evaluateRationalNURBSSurface(controlPoints, knotVectorU, degreeU, knotVectorV, degreeV, u, v);
...
int main() {
    ...
    std::vector<float> knotVectorU = {0, 0, 0, 1, 1, 1};
    ...
    std::vector<std::vector<Point>> surfacePoints = generateRationalNURBSSurface(controlPoints, knotVectorU, degreeU, knotVectorV, degreeV, numPointsU, numPointsV);
