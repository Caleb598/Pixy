...
Point evaluateBSplinePatch(const std::vector<std::vector<ControlPoint>>& controlPoints,
...
std::vector<std::vector<Point>> generateBSplinePatch(const std::vector<std::vector<ControlPoint>>& controlPoints,
                                                     const std::vector<float>& knotVectorU, int degreeU,
                                                     const std::vector<float>& knotVectorV, int degreeV,
    ...
    for (int i = 0; i <= numPointsU; ++i) {
        ...
        for (int j = 0; j <= numPointsV; ++j) {
            ...
            Point point = evaluateBSplinePatch(controlPoints, knotVectorU, degreeU, knotVectorV, degreeV, u, v);
...
int main() {
    ...
    std::vector<float> knotVectorV = {0, 0, 0, 0.5, 1, 1, 1};
    int degreeU = 3;
    int degreeV = 3;
    int numPointsU = 10;
    int numPointsV = 10;

    std::vector<std::vector<Point>> surfacePoints = generateBSplinePatch(controlPoints, knotVectorU, degreeU, knotVectorV, degreeV, numPointsU, numPointsV);
