...
Point evaluateNURBSPatch(const std::vector<std::vector<ControlPoint>>& controlPoints,
...
std::vector<std::vector<Point>> generateNURBSPatch(const std::vector<std::vector<ControlPoint>>& controlPoints,
                                                   const std::vector<float>& knotVectorU, int degreeU,
                                                   const std::vector<float>& knotVectorV, int degreeV,
    ...
    for (int i = 0; i <= numPointsU; ++i) {
        ...
        for (int j = 0; j <= numPointsV; ++j) {
            ...
            Point point = evaluateNURBSPatch(controlPoints, knotVectorU, degreeU, knotVectorV, degreeV, u, v);
...
int main() {
    ...
    std::vector<std::vector<Point>> surfacePoints = generateNURBSPatch(controlPoints, knotVectorU, degreeU, knotVectorV, degreeV, numPointsU, numPointsV);
