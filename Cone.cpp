...
void drawCone(GLfloat radius, GLfloat height, GLint slices, GLint stacks) {
    ...
    for (i = 0; i < slices; i++) {
        ...
        for (j = 0; j <= stacks; j++) {
            ...
            x = radius * (1 - phi / height) * cos(theta);
            y = radius * (1 - phi / height) * sin(theta);
            z = phi;

            glNormal3f(x, y, radius);
            glVertex3f(x, y, z);

            x = radius * (1 - (phi + height / stacks) / height) * cos(theta);
            y = radius * (1 - (phi + height / stacks) / height) * sin(theta);
            z = phi + height / stacks;
